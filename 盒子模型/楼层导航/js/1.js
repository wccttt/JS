window.onload=function () {
    ...
    let HH=utils.win("scrollHeight")-utils.win("clientHeight");
    ...
    function fn () {
        ...
        var H=utils.win("scrollTop");
        if(H>_L){
            timer=setInterval(()=> {
                //想让这里面的this仍然是外面的this,必须使用箭头函数
                H-=50;
                utils.win("scrollTop",H);
                if(utils.win("scrollTop")<=_L){
                    utils.win("scrollTop",_L);
                    clearInterval(timer)
                }
            },20);
        }else {
    ...
    window.onscroll=function () {
        ...
        for (let i=0;i<list.length-1;i++){
            ...
            if(T>=utils.offset(oDiv[8]).top-winH/2){
                select(list[8]);
            }
        }
    }
